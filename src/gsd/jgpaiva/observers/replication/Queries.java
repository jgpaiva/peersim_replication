...
public class Queries extends ControlImpl {
	...
	public Queries(String prefix) {
		...
		if (Configuration.contains(prefix + "." + Queries.PAR_DISTRIBUTION)) {
			this.distribution = Configuration.getString(prefix + "." + Queries.PAR_DISTRIBUTION);
		} else {
			this.distribution = null;
		}
		if (Configuration.contains(prefix + "." + Queries.PAR_ALTERNATIVE)) {
			this.alternativeMode = Configuration.getBoolean(prefix + "." + Queries.PAR_ALTERNATIVE);
	...
	protected boolean executeCycle() {
		...
		if (Network.get(0).getProtocol(this.pid) instanceof GroupReplication2) {
			...
			for (TreeSet<Node> group : groups) {
				...
				int keys = ((GroupReplication2) group.first().getProtocol(this.pid)).getKeys();
				keyStats.add(keys, group.size());
