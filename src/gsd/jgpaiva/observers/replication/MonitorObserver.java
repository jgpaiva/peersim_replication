...
public class MonitorObserver extends ControlImpl {

	public MonitorObserver(String prefix) {
		super(prefix);
	}
	...
	protected boolean executeCycle() {
		...
		for (int it = 0; it < Network.size(); it++) {
			MonitorableProtocol proto = (MonitorableProtocol) Network.get(it).getProtocol(this.pid);
			if (!proto.isUp()) {
				continue;
			}
			count++;
			stats.add(proto.getMonitoringCount());
		}
